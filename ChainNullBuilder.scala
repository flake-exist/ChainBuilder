object ChainNullBuilder {
  ...
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder.appName("Nullable Chains").getOrCreate()
...
//
    ...
    val data_preprocess_1 = data_preprocess_0.
      withColumn("goal",when($"HitTimeStamp" >= bonds.start,TRANSIT_ACTION).otherwise($"goal")).
...
//
    ...
    val data_seq = data_group.select(
      $"ClientID",
